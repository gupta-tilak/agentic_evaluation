...
def create_test_cases() -> List[LLMTestCase]:
    ...
    return [
        ...
        LLMTestCase(
            ...
            domain=DomainType.SUMMARIZATION.value
        ),
    ...
    return n * factorial(n-1)""",
            ...
            domain=DomainType.CODE_GENERATION.value
        )
    ]
...
def main():
    ...
    # Convert leaderboard to a dictionary format
    leaderboard_dict = {
        "rankings": [
            {
                ...
                "rank": rank.rank,
                "domain_scores": rank.domain_scores,
        ...
        "domain_rankings": {
            ...
            ] for domain, rankings in leaderboard.domain_rankings.items()
        },
        "top_performers": [
            {
                "agent_id": rank.agent_id,
                ...
                "percentile": rank.percentile,
                "trend": rank.trend
            } for rank in leaderboard.top_performers
        ],
        "most_improved": [
            {
                "agent_id": rank.agent_id,
                ...
                "percentile": rank.percentile,
                "trend": rank.trend
            } for rank in leaderboard.most_improved
        ],
        "most_declined": [
            {
                "agent_id": rank.agent_id,
                ...
                "percentile": rank.percentile,
                "trend": rank.trend
            } for rank in leaderboard.most_declined
        ],
        "created_at": leaderboard.created_at.isoformat()
    }
    print(json.dumps(leaderboard_dict, indent=2))
    ...
    for agent_id in agent_ids:
        try:
            ...
            print(str(explanation))  # Use the default string representation
        except Exception as e:
            print(f"\nCould not generate performance analysis for {agent_id}: {str(e)}")
    ...
    if isinstance(domain_analysis, dict):
        print(json.dumps(domain_analysis, indent=2))
    else:
        # If it's not a dictionary, convert it to a string representation
        print(str(domain_analysis))
